---
title: "RPS Hackathon @ Recurse: Test Results"
...
<script>
    ...
    async function initTable() {
            ...
            if (testing_data) {
                ...
                testingInfoContainer.innerHTML = `
                    <div class="tournament-info">
                        <strong>You: ${formatNumber(testing_data.mean)}</strong> | Started: ${formatDateMaybe(testing_data.start_time)} | 
        ...
        if (last_leaderboard_scoring_data["next_round_name"] === null) {
            ...
            document.getElementById('next-round-name').innerHTML = `"${last_leaderboard_scoring_data["next_round_name"]}" at ${formatDateMaybe(last_leaderboard_scoring_data["next_round_time"])}`;
        }
    }
    ...
    async function initHistories() {
        ...
        sortedRounds.forEach(roundDate => {
            ...
            summary.textContent = `Round ${roundDate}`;
            ...
            infoP.innerHTML = `Start: ${formatDate(tournamentInfo.start_time)}<br>End: ${formatDate(tournamentInfo.end_time)}`;
            content.appendChild(infoP);
    
            // Create tables for each matchup
            Object.keys(roundData.matchups).forEach(opponent => {
    ...
    function getColorForAction(action) {
        switch (action) {
            ...
            case 'P': return '#BAFFC9'; // Light Green for Paper
            case 'S': return '#BAE1FF'; // Light Blue for Scissors
