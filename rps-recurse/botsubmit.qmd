...
<div class="code-line"><textarea class="editable" id="imports" indent=0>
import random</textarea></div>
<div class="code-line"><pre><code class="language-python">
class Player(Bot):
</code></pre></div>
<div class="code-line"><pre><code class="language-python">    </code></pre><textarea class="editable" id="classlevel" indent=4>
...
<script>
  ...
  function initializeEditors() {
    const editors = [
      { id: 'imports', mode: 'python', height: '50px' },
      { id: 'classlevel', mode: 'python', height: '50px' },
      { id: 'init', mode: 'python', height: '100px' },
      { id: 'handle_results', mode: 'python', height: '100px' },
      { id: 'get_action', mode: 'python', height: '150px' }
    ];

    editors.forEach(editor => {
      const cm = CodeMirror.fromTextArea(document.getElementById(editor.id), {
        lineNumbers: true,
        mode: editor.mode,
        theme: "default",
        indentUnit: 4,
      ...
      cm.setSize(null, editor.height);
      
      // Store CodeMirror instance in the textarea's data attribute
      document.getElementById(editor.id).CodeMirror = cm;
    });
  }
  ...
  function autoSave() {
    ...
    editableFields.forEach((field) => {
      if (field.CodeMirror) {
        saveData[field.id] = field.CodeMirror.getValue();
      } else {
  ...
  function loadSavedData() {
    ...
    if (savedData) {
      ...
      Object.keys(parsedData).forEach((key) => {
        ...
        if (field && field.CodeMirror) {
          field.CodeMirror.setValue(parsedData[key]);
        } else if (field) {
    ...
    editableFields.forEach((field) => {
      if (field.CodeMirror) {
        data['snippets'][field.id] = {
          'contents': field.CodeMirror.getValue(),
