...
const indentedSections = [false, false, true, true, true, false];
...
function initializeEditor() {
    ...
    for (let i = 0; i < fixedCode.length; i++) {
        // Add fixed text
        addFixedText(codeEditor, fixedCode[i], indentedSections[i]);

        // Create editable box (if it's not the last fixed text)
        if (i < initialCode.length) {
            createEditor(codeEditor, i, indentedSections[i]);
        }
    }
}

function addFixedText(parent, text, isIndented) {
...
function createEditor(parent, index, isIndented) {
