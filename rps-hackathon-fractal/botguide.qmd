...
As we show at the [bottom of the page](#game-theory-equilibrium), playing a random strategy guarantees that you will break even in expectation. Since there are many suboptimal environment bots, this won't be a sufficient strategy, but could be valuable to consider in some situations. 
...
- If we're winning or tying, then we play to beat the action from 2 games ago 80% of the time and play randomly 20% of the time
- If we're losing, then we always play randomly

```python
# If we're losing, play randomly
if self.my_profit < 0:
    return random.choice([RockAction(), PaperAction(), ScissorsAction()])

# If we're winning or tying, and we have at least 2 rounds of history
...
Here's how we can execute this strategy: 
