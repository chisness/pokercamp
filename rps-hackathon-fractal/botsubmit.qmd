...
The file you upload should be a python file that begins:
<div class="interactive-python-container"><pre>from skeleton.actions import RockAction, PaperAction, ScissorsAction
from skeleton.bot import Bot
from skeleton.runner import parse_args, run_bot</pre></div>

and eventually calls `run_bot()` on an instance of a bot of a class that extends `Bot`, together with `parse_args()`, like so:
<div class="interactive-python-container"><pre>class Player(Bot):
    def __init__(self):
        ...
...
if __name__ == '__main__':
    run_bot(Player(), parse_args())</pre></div>
</div>
