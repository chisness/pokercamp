...
Which solver settings seem to help? Which don't? Which work best? What settings or logic do you wish you had that you don't have yet? We'll discuss in class and/or during challenge review and/or during office hours and/or on the discord.

In later weeks, you'll implement your own solver to do similar things on larger games, and while we'll try to give you bare-bones handout code, you'll be able to add what you want to make it solve better. Or not. ¯\\\_(ツ)\_/¯
:::
...
<button id="submit_strategy" style="padding: 0.5em 1em; color: white; background-color: #2fa4e7" onclick="submit_probabilities()">Update Strategy Submission</button>
...
<script>
  function fetchLeaderboard() {
    fetch('http://pokercamp-staff-dev1.rossry.net:5000/leaderboard')
      ...
      .then(data => {
        ...
        data.forEach(group => {
          ...
          groupHeader.innerHTML = `Places ${group.from_place}-${group.to_place} (Scores: ${group.from_score.toFixed(1)} to ${group.to_score.toFixed(1)})`;
