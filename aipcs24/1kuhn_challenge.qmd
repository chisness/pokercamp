...
Next, <span class="player p2"></span> will act. He can observe his card and <span class="player p1"></span>'s first action, but not <span class="player p1"></span>'s card. <span class="player p2"></span>'s 6 infosets are identified by IDs like <span class="p2 infoset">_K↓</span> and <span class="p2 infoset">_Q↑</span> after his card and previous actions, in this case <span class="player p1"></span>'s first action:
...
If the actions were <span class=action>↓↑</span>, then <span class="player p1"></span> will have to act again (equivalent to a Check and Bet). Otherwise, the game is now over (with a payoff determined by the cards and the action sequence). 
...
You can refer to the sequence table above to recall the 5 sequences of betting, which end either in one player folding or two <span class=action>↑</span> or <span class=action>↓</span> actions, meaning the high card wins at the *showdown*. 
...
Each player's strategy can be completely described in terms of their action probabilities at each infoset&mdash;6 of them total per player.
...
Consider <span class="player p2"></span>'s actions at <span class="infoset p2">_Q↓</span> (the poker situation of having a <span class=gamenode>Q</span> facing a Check). When <span class="player p1"></span> played <span class="infoset p1">K_↓↑</span> (the poker situation of having a <span class=gamenode>K</span> and Checking and then facing a Bet) randomly, <span class="player p2"></span> would get a payoff of <span class="player_label p2">-1.0</span> (<span class="player_label p1">+1.0</span>) for playing <span class=action>↓Down</span>(always Folding) and <span class="player_label p2">-0.5</span> (<span class="player_label p1">+0.5</span>) for playing <span class=action>↑Up</span> (always Calling). 

<span class="player p2"></span>'s <span class=gamenode>Q</span> is always has the worst hand, but if <span class="player p1"></span> plays <span class=action>↓Down</span> (Folds) often enough, then <span class="player p2"></span> will win <span class="player_label p2">+1</span>s often enough to come out ahead regardless.
