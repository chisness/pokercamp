...
List of all possible betting sequences. Sequences can either end with a <abbr title="Both players show cards and the highest wins">showdown</abbr> resulting from two ↑ (Bet and Call) or two ↓ actions (Check and Check), or ↑ and ↓ actions (Bet and Fold) such that the ↑ (betting) player automatically wins the pot. 
...
You can refer to the sequence table above to recall the 5 sequences of betting, which end either in one player folding (after <span class=action>↑</span> and <span class=action>↓</span> actions) or two <span class=action>↑</span> (Bet and Call) or <span class=action>↓</span> (Check and Check) actions, meaning the high card wins at the *showdown*. 
