---
title: "Kuhn Poker (1): Challenge (part 1)"
sidebar: aipcs24
format:
  html:
    math: true
...
Any deck of 3 ranked cards works. For example:
  ...
  - Wikipedia uses {Jack, Queen, King}.
:::

- Kuhn Poker is played with 2 players. (Nearly every game we will play in the first 10 classes of this course will be exactly 2 players.) For this exercise, they'll be <span class="player p1"></span> and <span class="player p2"></span>.
...
:::{.callout-tip collapse="true"  appearance="minimal"}
## <span class=action>↑Up</span> and <span class=action>↓Down</span> vs traditional poker terms

  - <span class=action>↑Up</span> actions indicate a <abbr title="To 'bet' is to add chips to pot that others have to match (or fold). To 'raise' is to put in a larger amount than your opponent's bet, to force then to match your higher amount (or fold).">Bet</abbr> or <abbr title="To 'call' is to match an opponent's bet.">Call</abbr>
...
3. A player plays <span class=action>↑Up</span>, then the other player plays <span class=action>↑Up</span>. They go to a showdown and the winner wins the two chips the loser has put in the pot (and their own chips back). Note that the game will proceed to a 2-chip showdown if the action is <span class=action>↑Up</span>, <span class=action>↑Up</span> or if the action is <span class=action>↓Down</span>, <span class=action>↑Up</span>, <span class=action>↑Up</span>.

An example game might go as follows:

- <span class="player p1"></span>: <span class=action>↑Up</span> (1 more chip into pot)

- <span class="player p2"></span>: <span class=action>↑Up</span> (1 more chip into pot)

- Both cards are revealed. <span class="player p1"></span> has an <span class=card>A</span> and <span class="player p2"></span> has a <span class=card>K</span>.
...
## Exercise: Get Started with Kuhn

If you have a partner handy, try the following:

- Play a few hands of Kuhn Poker, get used to the order of events in the game.

- Think about whether there are any situations where there is a clearly correct move that you should take 100% of the time. (This isn't a trick&mdash;there are such situations in this game.)
:::
...
For example, consider <span class="player p1"></span>'s decision at the infoset <span class="infoset p1">K_↓↑</span>. When <span class="player p1"></span> is at <span class="infoset p1">K_↓↑</span>, he doesn't know whether the true state of the world is <span class=gamenode>KA_↓↑</span> or <span class=gamenode>KQ_↓↑</span>. This is what the game looks like from <span class="player p1"></span>'s perspective: 

![](assets/k_down_up.png)

As we said earlier, whatever action (or randomized mix of actions) <span class="player p1"></span>'s strategy says to make, he will be doing so in all of the situations where he arrives at <span class="infoset p1">K_↓↑</span>, without the ability to do different things for the different underlying states.

<div class="duplicate_K_↓↑"></div>

<div class="duplicate_player_evs"></div>

If we start by assuming that both players play randomly (50% <span class=action>↑Up</span> on each action), then <span class="player p1"></span> would arrive at <span class="infoset p1">K_↓↑</span> via <span class=gamenode>KA↓↑</span> equally often as via <span class=gamenode>KQ↓↑</span>. In this case, his expected payoff for playing <span class=action>↑Up</span> is a 50%-50% weighted sum of the payoffs <span class=gamenode>KA↓↑↑</span> (<span class="player_label p1">-2</span>, calling with <span class=gamenode>K</span> when opponent has <span class=gamenode>A</span>) and <span class=gamenode>KQ↓↑↑</span> (<span class="player_label p1">+2</span>, calling with <span class=gamenode>K</span> when opponent has <span class=gamenode>Q</span>), for an expected value of <span class="player_label p1">0</span>. 
...
The goal of this challenge is to find Nash, or regret-free, strategies for Kuhn Poker in terms of the action probabilities at each player's 6 infosets. You will submit strategies for both players (12 probabilities total) and be matched against many opponents (but not yourself). For each opponent, you will play an equal number of times as <span class="player p1"></span> and as <span class="player p2"></span>.
...
When you get a pair of strategies such that each infoset is at most 0.1 chips of EV away from regret-free, a link to the next stage will appear below.
...
[Enter Part 2](1kuhn_challenge2.qmd)
