...
Thanks for signing up. This is the first time we're running AI Poker Camp and it's very much in beta. The idea of some form of this course has existed for a few years, but we didn't actually start planning this curriculum until about a month before launch time. 

We're glad to have you along for the ride and welcome any feedback about all aspects of the course. 

We're running this under the flipped classroom model, where we spend the majority of the time in the sessions focused on thinking about and working on the challenge problems, and time outside the sessions doing relevant readings and further work on the challenge problems. 

Our goal is to develop your intuitions around decision making and problem solving by engaging with challenges in non-idealized scenarios. 
...
:::{.callout-warning collapse="true"  appearance="minimal"}
## Solution
1. Both Fudd notes are a single information set because when Fudd is making the Hunt_Near or Hunt_Far decision, he doesn't know whether he's in the Opera or the Forest. 

2. Both Bugs nodes are also a single information set because although Bugs knows that he's in the Forest, he doesn't know which action Fudd has taken. 
...
:::{.callout-warning collapse="true"  appearance="minimal"}
## Solution
1. $\mathbb{E}(\text{Play_Near}) = -3*p + 1*(1-p) = -4*p + 1$

2.  $\mathbb{E}(\text{Play_Far}) = 0*p + -2*(1-p) = 2*p - 2$ 
...
:::{.callout-warning collapse="true"  appearance="minimal"}
## Solution
1. $\mathbb{E}(\text{Hunt_Near}) = 0.5*0 +0.5*[3*q + 0*(1-q)] = 1.5*q$

2.  $\mathbb{E}(\text{Hunt_Far}) = 0.5*(-1) + 0.5*[-1*q + 2*(1-q)] = -0.5 - 0.5*q + 1 - q = 0.5 - 1.5*q$
...
1. For Bugs to never choose an action with regret, EV of Hunt_Near and EV of Hunt_Far should be equal. 

$\mathbb{E}(\text{Hunt_Near}) = 1.5*q$

$\mathbb{E}(\text{Hunt_Far}) = 0.5 - 1.5*q$

Setting equal, we have $1.5*q = 0.5 - 1.5*q \Rightarrow 3*q = 0.5 \Rightarrow q = \frac{1}{6} = 0.167$

2. For Fudd to never choose an action with regret, EV of Play_Near and EV of Play_Far should be equal. 

$\mathbb{E}(\text{Play_Near}) = -4*p + 1$

$\mathbb{E}(\text{Play_Far}) = 2*p - 2$
